  ...
  return (
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      <div>
        ...
        >
          <FaMoon
            className={`absolute left-1 w-5 h-5 md:w-8 md:h-8 text-gray-200 ${theme === 'dark' ? 'block' : 'hidden'}`}
          />
          <span
            className={`${
              theme === 'dark'
          ...
          ></span>
          <FaSun
            className={`absolute right-1 w-5 h-5 ml-1 md:w-8 md:h-8 text-yellow-500 ${theme === 'dark' ? 'hidden' : 'block'}`}
          />
        </button>
        {children}
      </div>
