    ...
    <div>
      ...
      <main className="h-auto min-h-screen bg-[#f5f5f5] dark:bg-[#000000] pt-12 px-5 flex justify-center items-start overflow-auto">
        <div className="w-full max-w-6xl mb-12">
          <section className="container h-auto mx-auto bg-violet-300 dark:bg-[#f5f5f515] p-5 md:p-8 rounded-lg shadow-lg flex flex-col mb-8 md:mb-0 mt-8">
            <h1
            ...
            <div className="space-y-4 text-violet-900 dark:text-cyan-400">
              {experiences?.map(({ title, company_name, date, points }) => (
                <section
                  key={title}
                  className="bg-transparent p-4 rounded-lg shadow-sm"
                >
                  <div
                    className={`p-2 ${isClient ? 'animate-slide-from-right' : ''}`}
                  >
                  ...
                  <div
                    className={`p-2 ${isClient ? 'animate-slide-from-left' : ''}`}
                  >
