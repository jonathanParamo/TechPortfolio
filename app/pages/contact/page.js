...
const Notification = ({ status, type, onClose }) => {
  ...
  const bgColor =
    type === 'success' ? 'bg-purple-500 dark:bg-cyan-500' : 'bg-red-700';
  const textColor = 'text-white';

  return (
    status && (
      <div
        className={`fixed bottom-1 left-1/2 transform -translate-x-1/2 p-4 rounded-md shadow-lg ${bgColor} ${textColor} w-1/3 max-w-md text-center`}
      >
      ...
      <main className="min-h-screen bg-white dark:bg-[#000000] pt-12 px-5 flex justify-center items-start">
        <section className="container h-auto mx-auto bg-violet-300 dark:bg-[#f5f5f515] p-5 md:p-8 rounded-lg shadow-lg flex flex-col mb-4 md:mb-0 mt-8">
          ...
          <p className="dark:text-cyan-400 mb-2 text-sm md:text-lg text-violet-800">
            ...
            use the contact information provided to get in touch with us.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-violet-300 dark:bg-transparent p-4 md:p-6 rounded-lg shadow-md">
              <h2 className="text-base md:text-lg font-semibold text-purple-800 mb-4 dark:text-cyan-300">
                Get in Touch
              </h2>
              <form
                action="#"
              ...
              >
                <div>
                  <label
                    htmlFor="name"
                    className="block text-xs md:text-sm font-medium text-violet-900 dark:text-cyan-300"
                  >
                    Name
                  </label>
                  <input
                ...
                <div>
                  <label
                    htmlFor="message"
                    className="block text-sm font-medium text-violet-700 dark:text-cyan-300"
                  >
                    Message
                  </label>
                  <textarea
            ...
            <div className="bg-violet-300 dark:bg-transparent p-4 md:p-6 rounded-lg shadow-md">
              <h2 className="text-md md:text-xl font-semibold text-violet-900 dark:text-cyan-400 mb-4">
